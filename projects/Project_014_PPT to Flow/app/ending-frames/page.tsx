...
type EndingScene = FlowScene & {
  endingFrameReferenceName?: string;
  endingFrameReferenceDataUrl?: string;
};

const endingStatuses: FlowScene["endingFrameStatus"][] = [
  "not_available",
  "available",
  "needs_revision",
];
...
export default function EndingFramesPage() {
  ...
  const [scenes, setScenes] = useState<EndingScene[]>([]);
  const [selectedSceneId, setSelectedSceneId] = useState("");
  const [note, setNote] = useState("");
  const [status, setStatus] = useState<FlowScene["endingFrameStatus"]>("not_available");
  ...
  useEffect(() => {
    async function loadScenes() {
      ...
      const loadedScenes = (await db.flowScenes
        ...
        .sortBy("sceneNumber")) as EndingScene[];
  ...
  const nextScene = useMemo(() => {
    ...
    return scenes.find((scene) => scene.sceneNumber === selectedScene.sceneNumber + 1) ?? null;
  ...
  useEffect(() => {
    ...
    setNote(selectedScene.endingFrameNote || "");
    setStatus(selectedScene.endingFrameStatus);
    setImageName(selectedScene.endingFrameReferenceName || "");
    setImageDataUrl(selectedScene.endingFrameReferenceDataUrl || "");
    setMessage(null);
    setCopied(false);
  }, [selectedScene]);
  ...
  const onImageChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (status === "not_available") setStatus("available");
  };

  const saveEndingFrame = async () => {
    if (!selectedScene) return;
    await db.flowScenes.update(selectedScene.id, {
      endingFrameNote: note,
      endingFrameStatus: status,
      endingFrameReferenceName: imageName || undefined,
      endingFrameReferenceDataUrl: imageDataUrl || undefined,
              ...
              <div className="mt-4 space-y-2">
                {scenes.map((scene) => (
                  ...
                  >
                    ...
                    <StatusBadge status={scene.endingFrameStatus} />
                  </button>
                ))}
              </div>
            </AppCard>
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <p className="mt-3 text-sm font-semibold text-text">
                        {labels.nextScene}: {nextScene ? sceneNames[nextScene.id] : labels.finalScene}
                      </p>
                    </div>
                  ...
                  <AppCard>
                    <div className="grid gap-4">
                      <FormSelect
                        ...
                        value={status}
                        options={endingStatuses}
                        onChange={(value) => setStatus(value as FlowScene["endingFrameStatus"])}
                      />
                      <FormTextarea label={labels.noteLabel} value={note} onChange={setNote} />
                    </div>
                    <div className="mt-6 flex flex-wrap gap-3">
                      <AppButton type="button" onClick={saveEndingFrame}>
                        <Save className="h-4 w-4" aria-hidden="true" />
                        {labels.save}
                      </AppButton>
                      <AppButton type="button" variant="outline" onClick={copyNote} disabled={!note.trim()}>
...
function StatusBadge({ status }: { status: FlowScene["endingFrameStatus"] }) {
  const variant = status === "available" ? "success" : status === "needs_revision" ? "warning" : "neutral";
...
const copy = {
  en: {
    title: "Ending Frames",
    description: "Track whether each ending frame is available so every Flow Scene can bridge cleanly into the next shot.",
    emptyTitle: "Import or create a Flow Project first.",
    emptyDescription: "Ending frames are managed per scene after a project has scenes.",
    ...
    nextScene: "Next Flow Scene",
    finalScene: "Final scene / no next scene",
    referenceImage: "Ending Frame Reference",
    uploadImage: "Import Ending Frame",
    uploadHelp: "Use the final frame from this scene, or the image that should bridge into the next scene.",
    statusLabel: "Ending Frame Status",
    noteLabel: "Ending Frame Note",
    save: "Save Ending Frame",
    copyNote: "Copy Note",
    saved: "Ending frame reference saved.",
  },
  "zh-TW": {
    title: "結尾畫面",
    description: "追蹤結尾畫面是否可用，確保每個 Flow 場景能順暢銜接下一個鏡頭。",
    emptyTitle: "請先匯入或建立 Flow 專案。",
    emptyDescription: "專案有場景後，就可以逐場管理結尾畫面。",
    goImport: "前往多模態匯入",
    ...
    currentScene: "目前場景",
    nextScene: "下一個 Flow 場景",
    finalScene: "最後一幕 / 沒有下一幕",
    referenceImage: "結尾畫面參考",
    uploadImage: "匯入結尾畫面",
    uploadHelp: "使用此場景最後一幀，或要銜接到下一幕的橋接畫面。",
    statusLabel: "結尾畫面狀態",
    noteLabel: "結尾畫面備註",
    save: "儲存結尾畫面",
    copyNote: "複製備註",
