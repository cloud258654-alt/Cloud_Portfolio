...
1. **`solve_linear_regression_crispdm.py`**: The main execution orchestrator containing all modular functions (`data_understanding`, `build_pipeline`, `evaluate_train_test`, `evaluate_cross_validation`, `run_model_experiments`, `select_final_model`, `deployment_simulation`, `save_model`).
2. **`linear_regression_model.pkl`**: The serialized final pipeline (preprocessor + regression weights) saved using `joblib`.
...
We implemented and plotted **all 9 top feature selection algorithms** in a unified comparison visual ([feature_selection_performance_allinone.png](./feature_selection_performance_allinone.png)):
