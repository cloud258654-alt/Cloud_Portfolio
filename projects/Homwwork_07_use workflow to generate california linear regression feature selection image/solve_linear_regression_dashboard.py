...
# Preprocessing Pipeline (Standardize all variables)
...
if show_comparison:
        ...
        if "Linear" in model_type or "Lasso" in model_type:
            st.markdown("""
                <div style="background-color: rgba(129, 140, 248, 0.08); border-left: 4px solid #818cf8; padding: 15px; border-radius: 4px; margin-top: 15px;">
                    ...
                    Coefficients represent the change in target value (medv in $1000s) per standard deviation change of the feature.
                    Standardization helps prevent columns with larger scales (like tax) from artificially dominating weights.
                </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown("""
    ...
    with sim_col_left:
        st.markdown("##### 1. Input Property Specifications")
        crim = st.number_input("crim (Per capita crime rate)", min_value=0.0, max_value=100.0, value=0.1, step=0.05)
        zn = st.slider("zn (Res. land zoned for lots >25k sq.ft. %)", min_value=0.0, max_value=100.0, value=12.5, step=1.0)
        indus = st.slider("indus (Non-retail business acres %)", min_value=0.0, max_value=30.0, value=7.8, step=0.5)
        chas_val = st.selectbox("chas (Bounds Charles River)", ["No", "Yes"])
        chas = 1 if chas_val == "Yes" else 0
        nox = st.slider("nox (Nitric oxides conc. parts/10M)", min_value=0.3, max_value=0.9, value=0.53, step=0.01)
        rm = st.slider("rm (Avg rooms per dwelling)", min_value=3.0, max_value=9.0, value=6.5, step=0.1)
        age = st.slider("age (Owner units built prior to 1940 %)", min_value=0.0, max_value=100.0, value=65.0, step=1.0)
        dis = st.slider("dis (Weighted dist to employment centres)", min_value=1.0, max_value=12.0, value=4.0, step=0.1)
        rad = st.slider("rad (Index of highway accessibility)", min_value=1, max_value=24, value=4, step=1)
        tax = st.number_input("tax (Full tax rate per $10k)", min_value=180.0, max_value=720.0, value=300.0, step=10.0)
        ptratio = st.slider("ptratio (Pupil-teacher ratio)", min_value=12.0, max_value=23.0, value=18.0, step=0.2)
        b = st.slider("b (1000(Bk - 0.63)^2 population metric)", min_value=0.0, max_value=400.0, value=396.9, step=10.0)
        lstat = st.slider("lstat (% lower status of population)", min_value=1.0, max_value=40.0, value=12.0, step=0.5)
